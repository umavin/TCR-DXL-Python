...
class dxl_comms:
    ...
    def shutdown(self):
        ...
        for id in self.DXL_ID:
            ...
            dxl_comm_result, dxl_error = self.packetHandler.write1ByteTxRx(self.portHandler, id, self.ADDR_XM_TORQUE_ENABLE, self.TORQUE_DISABLE)
            if dxl_comm_result != COMM_SUCCESS:
                print("%s" % self.packetHandler.getTxRxResult(dxl_comm_result))
            elif dxl_error != 0:
                print("%s" % self.packetHandler.getRxPacketError(dxl_error))
    ...
    def set_profile_acceleration(self,prof_acc):
        for id in self.DXL_ID:
